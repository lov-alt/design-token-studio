...
const NAV = [
  { path: "/colors", key: "colors" as const },
  { path: "/typography", key: "typography" as const },
  { path: "/spacing", key: "spacing" as const },
  { path: "/accessible", key: "accessible" as const },
...
export default function App() {
  ...
  const [dark, setDark] = useState(() =>
    localStorage.getItem("token-studio-theme") === "dark"
  );

  useEffect(() => {
    document.body.classList.toggle("dark", dark);
    localStorage.setItem("token-studio-theme", dark ? "dark" : "light");
  }, [dark]);

  return (
    <div className="min-h-screen bg-zinc-50 dark:bg-[#0f0f1a] transition-colors duration-300">
      <header className={`${isHome ? "" : "border-b border-zinc-200/60 dark:border-zinc-800/60"} bg-white/80 dark:bg-[#0f0f1a]/80 backdrop-blur-xl sticky top-0 z-50 transition-colors`}>
        <div className="max-w-6xl mx-auto px-6 h-14 flex items-center justify-between">
          <Link to="/" className="font-medium tracking-tight text-zinc-900 dark:text-zinc-100 hover:text-indigo-600 dark:hover:text-indigo-400 transition-colors">
            {t.app.title}
          </Link>
